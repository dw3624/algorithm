...
# 이진 트리의 중위 순회(inorder traversal)와 후위 순회(postorder traversal)가 주어질 때,
# 이를 전위 순회(preorder traversal)로 출력하는 문제입니다.
# 전위 순회: root → left → right
# 중위 순회: left → root → right
# 후위 순회: left → right → root
#
# 후위 순회에서는 트리의 root를 알 수 있고, 중위 순회에서는 좌우 자식 트리의 정보를 알 수 있습니다.
# 후위 순회의 마지막 원소는 최상위 루트이며,
# 이를 기준으로 중위 순회를 왼쪽 서브 트리와 오른쪽 서브 트리로 나눌 수 있습니다.
#
# 예)
# 중위 순회 : 4 2 5  ((1))  6 3 7
# 후위 순회 : 4 5 2 6 7 3  ((1))
